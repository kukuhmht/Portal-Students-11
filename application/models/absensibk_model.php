<?php
// Author: Kukuh MHT
class Absensibk_model extends CI_Model {
	...
	public function data_absensi($idkelas) {
		return $this->db->query('SELECT *FROM siswa WHERE id_kelas="'.$idkelas.'"  ORDER BY nama_siswa ASC');
	}
	...
	public function get_tanggal_alfa_januari($idkelas){
		return $this->db->query('
			...
			WHERE absensi.id_kelas = "'.$idkelas.'" AND bulan = "Januari" AND absen= "Alfa"
		');
	}
	public function get_tanggal_alfa_februari($idkelas){
		return $this->db->query('
			SELECT siswa.nama_siswa, absensi.tanggal, absensi.bulan, absensi.tahun, absensi.keterangan
			FROM absensi
			JOIN siswa ON absensi.id_siswa = siswa.ID
			WHERE absensi.id_kelas = "'.$idkelas.'" AND  bulan = "Februari" AND absen= "Alfa"
		');
	}
	public function get_tanggal_alfa_maret($idkelas){
		return $this->db->query('
			SELECT siswa.nama_siswa, absensi.tanggal, absensi.bulan, absensi.tahun, absensi.keterangan
			FROM absensi
			JOIN siswa ON absensi.id_siswa = siswa.ID
			WHERE absensi.id_kelas = "'.$idkelas.'" AND  bulan = "Maret" AND absen= "Alfa"
		');
	}
	public function get_tanggal_alfa_april($idkelas){
		return $this->db->query('
			SELECT siswa.nama_siswa, absensi.tanggal, absensi.bulan, absensi.tahun, absensi.keterangan
			FROM absensi
			JOIN siswa ON absensi.id_siswa = siswa.ID
			WHERE absensi.id_kelas = "'.$idkelas.'" AND  bulan = "April" AND absen= "Alfa"
		');
	}
	public function get_tanggal_alfa_mei($idkelas){
		return $this->db->query('
			SELECT siswa.nama_siswa, absensi.tanggal, absensi.bulan, absensi.tahun, absensi.keterangan
			FROM absensi
			JOIN siswa ON absensi.id_siswa = siswa.ID
			WHERE absensi.id_kelas = "'.$idkelas.'" AND  bulan = "Mei" AND absen= "Alfa"
		');
	}
	public function get_tanggal_alfa_juni($idkelas){
		return $this->db->query('
			SELECT siswa.nama_siswa, absensi.tanggal, absensi.bulan, absensi.tahun, absensi.keterangan
			FROM absensi
			JOIN siswa ON absensi.id_siswa = siswa.ID
			WHERE absensi.id_kelas = "'.$idkelas.'" AND  bulan = "Juni" AND absen= "Alfa"
		');
	}
	public function get_tanggal_alfa_juli($idkelas){
		return $this->db->query('
			SELECT siswa.nama_siswa, absensi.tanggal, absensi.bulan, absensi.tahun, absensi.keterangan
			FROM absensi
			JOIN siswa ON absensi.id_siswa = siswa.ID
			WHERE absensi.id_kelas = "'.$idkelas.'" AND  bulan = "Juli" AND absen= "Alfa"
		');
	}
	public function get_tanggal_alfa_agustus($idkelas){
		return $this->db->query('
			SELECT siswa.nama_siswa, absensi.tanggal, absensi.bulan, absensi.tahun, absensi.keterangan
			FROM absensi
			JOIN siswa ON absensi.id_siswa = siswa.ID
			WHERE absensi.id_kelas = "'.$idkelas.'" AND  bulan = "Agustus" AND absen= "Alfa"
		');
	}
	public function get_tanggal_alfa_september($idkelas){
		return $this->db->query('
			SELECT siswa.nama_siswa, absensi.tanggal, absensi.bulan, absensi.tahun, absensi.keterangan
			FROM absensi
			JOIN siswa ON absensi.id_siswa = siswa.ID
			WHERE absensi.id_kelas = "'.$idkelas.'" AND  bulan = "September" AND absen= "Alfa"
		');
	}
	public function get_tanggal_alfa_oktober($idkelas){
		return $this->db->query('
			SELECT siswa.nama_siswa, absensi.tanggal, absensi.bulan, absensi.tahun, absensi.keterangan
			FROM absensi
			JOIN siswa ON absensi.id_siswa = siswa.ID
			WHERE absensi.id_kelas = "'.$idkelas.'" AND  bulan = "Oktober" AND absen= "Alfa"
		');
	}
	public function get_tanggal_alfa_november($idkelas){
		return $this->db->query('
			SELECT siswa.nama_siswa, absensi.tanggal, absensi.bulan, absensi.tahun, absensi.keterangan
			FROM absensi
			JOIN siswa ON absensi.id_siswa = siswa.ID
			WHERE absensi.id_kelas = "'.$idkelas.'" AND  bulan = "November" AND absen= "Alfa"
		');
	}
	public function get_tanggal_alfa_desember($idkelas) {
		return $this->db->query('
			SELECT siswa.nama_siswa, absensi.tanggal, absensi.bulan, absensi.tahun, absensi.keterangan
			FROM absensi
			JOIN siswa ON absensi.id_siswa = siswa.ID
			WHERE absensi.id_kelas = "'.$idkelas.'" AND  bulan = "Desember" AND absen= "Alfa"
		');
	}

	// Menampilkan Siswa yang Sakit bulan Januari - Desember
	public function get_tanggal_sakit_januari($idkelas){
		return $this->db->query('
			SELECT siswa.nama_siswa, absensi.tanggal, absensi.bulan, absensi.tahun, absensi.keterangan
			FROM absensi
			JOIN siswa ON absensi.id_siswa = siswa.ID
			WHERE absensi.id_kelas = "'.$idkelas.'" AND  bulan = "Januari" AND absen= "Sakit"
		');
	}
	public function get_tanggal_sakit_februari($idkelas){
		return $this->db->query('
			SELECT siswa.nama_siswa, absensi.tanggal, absensi.bulan, absensi.tahun, absensi.keterangan
			FROM absensi
			JOIN siswa ON absensi.id_siswa = siswa.ID
			WHERE absensi.id_kelas = "'.$idkelas.'" AND  bulan = "Februari" AND absen= "Sakit"
		');
	}
	public function get_tanggal_sakit_maret($idkelas){
		return $this->db->query('
			SELECT siswa.nama_siswa, absensi.tanggal, absensi.bulan, absensi.tahun, absensi.keterangan
			FROM absensi
			JOIN siswa ON absensi.id_siswa = siswa.ID
			WHERE absensi.id_kelas = "'.$idkelas.'" AND  bulan = "Maret" AND absen= "Sakit"
		');
	}
	public function get_tanggal_sakit_april($idkelas){
		return $this->db->query('
			SELECT siswa.nama_siswa, absensi.tanggal, absensi.bulan, absensi.tahun, absensi.keterangan
			FROM absensi
			JOIN siswa ON absensi.id_siswa = siswa.ID
			WHERE absensi.id_kelas = "'.$idkelas.'" AND  bulan = "April" AND absen= "Sakit"
		');
	}
	public function get_tanggal_sakit_mei($idkelas){
		return $this->db->query('
			SELECT siswa.nama_siswa, absensi.tanggal, absensi.bulan, absensi.tahun, absensi.keterangan
			FROM absensi
			JOIN siswa ON absensi.id_siswa = siswa.ID
			WHERE absensi.id_kelas = "'.$idkelas.'" AND  bulan = "Mei" AND absen= "Sakit"
		');
	}
	public function get_tanggal_sakit_juni($idkelas){
		return $this->db->query('
			SELECT siswa.nama_siswa, absensi.tanggal, absensi.bulan, absensi.tahun, absensi.keterangan
			FROM absensi
			JOIN siswa ON absensi.id_siswa = siswa.ID
			WHERE absensi.id_kelas = "'.$idkelas.'" AND  bulan = "Juni" AND absen= "Sakit"
		');
	}
	public function get_tanggal_sakit_juli($idkelas){
		return $this->db->query('
			SELECT siswa.nama_siswa, absensi.tanggal, absensi.bulan, absensi.tahun, absensi.keterangan
			FROM absensi
			JOIN siswa ON absensi.id_siswa = siswa.ID
			WHERE absensi.id_kelas = "'.$idkelas.'" AND  bulan = "Juli" AND absen= "Sakit"
		');
	}
	public function get_tanggal_sakit_agustus($idkelas){
		return $this->db->query('
			SELECT siswa.nama_siswa, absensi.tanggal, absensi.bulan, absensi.tahun, absensi.keterangan
			FROM absensi
			JOIN siswa ON absensi.id_siswa = siswa.ID
			WHERE absensi.id_kelas = "'.$idkelas.'" AND  bulan = "Agustus" AND absen= "Sakit"
		');
	}
	public function get_tanggal_sakit_september($idkelas){
		return $this->db->query('
			SELECT siswa.nama_siswa, absensi.tanggal, absensi.bulan, absensi.tahun, absensi.keterangan
			FROM absensi
			JOIN siswa ON absensi.id_siswa = siswa.ID
			WHERE absensi.id_kelas = "'.$idkelas.'" AND  bulan = "September" AND absen= "Sakit"
		');
	}
	public function get_tanggal_sakit_oktober($idkelas){
		return $this->db->query('
			SELECT siswa.nama_siswa, absensi.tanggal, absensi.bulan, absensi.tahun, absensi.keterangan
			FROM absensi
			JOIN siswa ON absensi.id_siswa = siswa.ID
			WHERE absensi.id_kelas = "'.$idkelas.'" AND  bulan = "Oktober" AND absen= "Sakit"
		');
	}
	public function get_tanggal_sakit_november($idkelas){
		return $this->db->query('
			SELECT siswa.nama_siswa, absensi.tanggal, absensi.bulan, absensi.tahun, absensi.keterangan
			FROM absensi
			JOIN siswa ON absensi.id_siswa = siswa.ID
			WHERE absensi.id_kelas = "'.$idkelas.'" AND  bulan = "November" AND absen= "Sakit"
		');
	}
	public function get_tanggal_sakit_desember($idkelas) {
		return $this->db->query('
			SELECT siswa.nama_siswa, absensi.tanggal, absensi.bulan, absensi.tahun, absensi.keterangan
			FROM absensi
			JOIN siswa ON absensi.id_siswa = siswa.ID
			WHERE absensi.id_kelas = "'.$idkelas.'" AND  bulan = "Desember" AND absen= "Sakit"
		');
	}

	// Menampilkan Siswa yang Izin bulan Januari - Desember
	public function get_tanggal_izin_januari($idkelas){
		return $this->db->query('
			SELECT siswa.nama_siswa, absensi.tanggal, absensi.bulan, absensi.tahun, absensi.keterangan
			FROM absensi
			JOIN siswa ON absensi.id_siswa = siswa.ID
			WHERE absensi.id_kelas = "'.$idkelas.'" AND  bulan = "Januari" AND absen= "Izin"
		');
	}
	public function get_tanggal_izin_februari($idkelas){
		return $this->db->query('
			SELECT siswa.nama_siswa, absensi.tanggal, absensi.bulan, absensi.tahun, absensi.keterangan
			FROM absensi
			JOIN siswa ON absensi.id_siswa = siswa.ID
			WHERE absensi.id_kelas = "'.$idkelas.'" AND  bulan = "Februari" AND absen= "Izin"
		');
	}
	public function get_tanggal_izin_maret($idkelas){
		return $this->db->query('
			SELECT siswa.nama_siswa, absensi.tanggal, absensi.bulan, absensi.tahun, absensi.keterangan
			FROM absensi
			JOIN siswa ON absensi.id_siswa = siswa.ID
			WHERE absensi.id_kelas = "'.$idkelas.'" AND  bulan = "Maret" AND absen= "Izin"
		');
	}
	public function get_tanggal_izin_april($idkelas){
		return $this->db->query('
			SELECT siswa.nama_siswa, absensi.tanggal, absensi.bulan, absensi.tahun, absensi.keterangan
			FROM absensi
			JOIN siswa ON absensi.id_siswa = siswa.ID
			WHERE absensi.id_kelas = "'.$idkelas.'" AND  bulan = "April" AND absen= "Izin"
		');
	}
	public function get_tanggal_izin_mei($idkelas){
		return $this->db->query('
			SELECT siswa.nama_siswa, absensi.tanggal, absensi.bulan, absensi.tahun, absensi.keterangan
			FROM absensi
			JOIN siswa ON absensi.id_siswa = siswa.ID
			WHERE absensi.id_kelas = "'.$idkelas.'" AND  bulan = "Mei" AND absen= "Izin"
		');
	}
	public function get_tanggal_izin_juni($idkelas){
		return $this->db->query('
			SELECT siswa.nama_siswa, absensi.tanggal, absensi.bulan, absensi.tahun, absensi.keterangan
			FROM absensi
			JOIN siswa ON absensi.id_siswa = siswa.ID
			WHERE absensi.id_kelas = "'.$idkelas.'" AND  bulan = "Juni" AND absen= "Izin"
		');
	}
	public function get_tanggal_izin_juli($idkelas){
		return $this->db->query('
			SELECT siswa.nama_siswa, absensi.tanggal, absensi.bulan, absensi.tahun, absensi.keterangan
			FROM absensi
			JOIN siswa ON absensi.id_siswa = siswa.ID
			WHERE absensi.id_kelas = "'.$idkelas.'" AND  bulan = "Juli" AND absen= "Izin"
		');
	}
	public function get_tanggal_izin_agustus($idkelas){
		return $this->db->query('
			SELECT siswa.nama_siswa, absensi.tanggal, absensi.bulan, absensi.tahun, absensi.keterangan
			FROM absensi
			JOIN siswa ON absensi.id_siswa = siswa.ID
			WHERE absensi.id_kelas = "'.$idkelas.'" AND  bulan = "Agustus" AND absen= "Izin"
		');
	}
	public function get_tanggal_izin_september($idkelas){
		return $this->db->query('
			SELECT siswa.nama_siswa, absensi.tanggal, absensi.bulan, absensi.tahun, absensi.keterangan
			FROM absensi
			JOIN siswa ON absensi.id_siswa = siswa.ID
			WHERE absensi.id_kelas = "'.$idkelas.'" AND  bulan = "September" AND absen= "Izin"
		');
	}
	public function get_tanggal_izin_oktober($idkelas){
		return $this->db->query('
			SELECT siswa.nama_siswa, absensi.tanggal, absensi.bulan, absensi.tahun, absensi.keterangan
			FROM absensi
			JOIN siswa ON absensi.id_siswa = siswa.ID
			WHERE absensi.id_kelas = "'.$idkelas.'" AND  bulan = "Oktober" AND absen= "Izin"
		');
	}
	public function get_tanggal_izin_november($idkelas){
		return $this->db->query('
			SELECT siswa.nama_siswa, absensi.tanggal, absensi.bulan, absensi.tahun, absensi.keterangan
			FROM absensi
			JOIN siswa ON absensi.id_siswa = siswa.ID
			WHERE absensi.id_kelas = "'.$idkelas.'" AND  bulan = "November" AND absen= "Izin"
		');
	}
	public function get_tanggal_izin_desember($idkelas) {
		return $this->db->query('
			SELECT siswa.nama_siswa, absensi.tanggal, absensi.bulan, absensi.tahun, absensi.keterangan
			FROM absensi
			JOIN siswa ON absensi.id_siswa = siswa.ID
			WHERE absensi.id_kelas = "'.$idkelas.'" AND  bulan = "Desember" AND absen= "Izin"
		');
	}
